...
from pydantic_types.file_scan_state import FileScanState
import os
...
def scan_all_files():
    ...
    while len(state['file_prefixes_to_explore']) > 0:
        ...
        meta_dict = first_chunk.metadata.model_dump()   # or .dict() depending on your Pydantic version
        # pretty-print the metadata as JSON:
        meta_json = json.dumps(meta_dict, indent=2)

        # build a single content string that includes metadata + the page_content
        message_content = f"""
        ...
        state['files_current_prefix'].extend(chunks)
        print(f"scanning file with prefix: {cur_prefix}")
        # For each chunk, scan with the find_issues_agent
        while len(state['files_current_prefix']) > 0:
            cur_file = state['files_current_prefix'].pop()
